 ...
 * GET  /download increments downloads, serves gzip via ASSETS.fetch (no 302)
 * GET  /count   JSON {project, views, downloads, total}
 * /v1, /mcp, and /v1/mesh/* do not increment.
 *
 * Isolated counter: Worker embryolock-download-tracker, project embryolock.
 * Hosted /v1 never stores phrases or vault blobs and never executes wipe/scorch.
 * Author: Aziel Eliab only.
 */

const PROJECT = "embryolock";
const DEFAULT_ASSET = "embryolock-1.2.0.tar.gz";
const DEFAULT_OWNER = "AzielEliab";
const DEFAULT_REPO = "EmbryoLock";
...
async function kvGet(env, key) {
  ...
  return env.DOWNLOADS.get(key);
}

async function kvPut(env, key, value) {
  if (!env || !env.DOWNLOADS) return;
  await env.DOWNLOADS.put(key, value);
}

async function increment(env, dims) {
  const key = kvKey(dims);
  const n = parseInt((await kvGet(env, key)) || "0", 10) + 1;
  await kvPut(env, key, String(n));
  const tot = parseInt((await kvGet(env, totalKey())) || "0", 10) + 1;
  await kvPut(env, totalKey(), String(tot));
  return tot;
}

async function incrementViews(env) {
  const n = parseInt((await kvGet(env, viewsKey())) || "0", 10) + 1;
  await kvPut(env, viewsKey(), String(n));
  return n;
}

async function listAllKeys(env) {
  const keys = [];
  if (!env || !env.DOWNLOADS || typeof env.DOWNLOADS.list !== "function") return keys;
  let cursor;
  do {
...
async function githubStats(env) {
  const cached = await kvGet(env, githubCacheKey());
  if (cached) {
    try {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 EmbryoLock-download-tracker", Accept: "application/vnd.github+json" };
  let stars = 0;
  let forks = 0;
  let watchers = 0;
  let release_download_count = 0;
  try {
    const repoRes = await fetch("https://api.github.com/repos/AzielEliab/EmbryoLock", { headers });
    if (repoRes.ok) {
    ...
    const relRes = await fetch("https://api.github.com/repos/AzielEliab/EmbryoLock/releases/latest", { headers });
    if (relRes.ok) {
  ...
  try {
    await kvPut(env, githubCacheKey(), JSON.stringify(out));
  } catch {
    /* ignore */
  }
  return out;
...
async function collectStats(env) {
  ...
  for (const k of keys) {
    ...
    const n = parseInt((await kvGet(env, name)) || "0", 10);
  ...
  const totalDirect = parseInt((await kvGet(env, totalKey())) || "0", 10);
  const views = parseInt((await kvGet(env, viewsKey())) || "0", 10) || 0;
  ...
  return {
    ...
    note: "Forks identified by GitHub owner/repo. Key layout: project|owner|repo|branch|fork. /v1 does not increment.",
  };
}

function installScript() {
  return `#!/usr/bin/env bash
# EmbryoLock one-click install. Counted download via this Worker.
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/" && request.method === "GET") {
      ...
      return new Response(await renderHome(await collectStats(env)), {
