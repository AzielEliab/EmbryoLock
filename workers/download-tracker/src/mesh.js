...
export const MESH_PRODUCT = "embryolock";
...
export function meshPointer() {
  return {
    ...
    origin: RUNTIME + MESH_PATH,
    note: "PROXY to aziel-runtime /v1/mesh/* via AZIEL_RUNTIME. Not a local op. Not AnonBroadcast. Not AZMail's product-local ring. EmbryoLock remains a local destructive vault. Full node process is local qnm-node/. QNS-CD-1.0 photon QNS1 packet transfer is a hub cite / Worker mesh cross-map only — no public qnsd proxy. GET never enables. " + MESH_NOTE,
...
export function meshOpenApiPaths() {
  ...
  for (const route of MESH_PROXY_ROUTES) {
    ...
    for (const method of route.methods) {
      entry[method] = {
        operationId: "embryolock_mesh_" + route.op + (method === "head" ? "_head" : "") + "_proxy",
...
export async function originFetch(env, pathAndQuery, init, request) {
  ...
  headers.set("X-Aziel-Runtime-Via", "embryolock-download-tracker");
